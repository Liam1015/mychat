...
def setup_train_args():
    ...
    parser.add_argument('--model_config', default='config/model_config_dialogue.json', type=str, required=False,
                        help='选择模型参数')
    ...
    parser.add_argument('--train_tokenized_path', default='data/train_tokenized_word.txt', type=str,
                        required=False,
...
def preprocess_raw_data(args, tokenizer, n_ctx):
    ...
    with codecs.open(args.train_raw_path, 'r') as f:
        data = f.read()
    if "\n\n" in data:
    ...
    with codecs.open(args.train_tokenized_path, "w", encoding="utf-8") as f:
...
def preprocess_mmi_raw_data(args, tokenizer, n_ctx):
    ...
    if "\n\n" in data:
        ...
        train_data = data.split("\r\n\r\n")
    logger.info("there are {} dialogue in raw dataset".format(len(train_data)))
    with codecs.open(args.train_mmi_tokenized_path, "w", encoding="utf-8") as f:
        for dialogue_index, dialogue in enumerate(tqdm(train_data)):
            if "\n" in data:
                utterances = dialogue.split("\n")
            else:
            ...
            try:
                ...
                if (batch_idx + 1) % args.gradient_accumulation == 0:
                    ...
                    if (overall_step + 1) % args.log_step == 0:
                        logger.info(
                            "batch {} of epoch {}, step{}, loss {}, accuracy {}".format(batch_idx + 1, epoch + 1, overall_step,
                                                                                        loss, accuracy))
                        tb_writer.add_scalar('loss', loss.item(), overall_step)
                        tb_writer.add_scalar('accuracy', accuracy.item(), overall_step)
